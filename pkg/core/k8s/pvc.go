...
func ListPvc(c *gin.Context) {
	...
	pvcData, err := clientSet.CoreV1().PersistentVolumeClaims(namespaceOptions.Namespace).List(context.TODO(), metaV1.ListOptions{})
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"code": -1, "msg": err.Error()})
		return
	}
...
func GetPvc(c *gin.Context) {
	...
	pvcDetail, err := clientSet.CoreV1().PersistentVolumeClaims(requestOptions.Namespace).Get(context.TODO(), requestOptions.Name, metaV1.GetOptions{})
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"code": -1, "msg": err.Error(), "data": ""})
		return
	}

	c.JSON(http.StatusOK, gin.H{"code": 0, "msg": "操作成功", "data": pvcDetail})
}
